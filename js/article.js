...
//评论模板
	...
	var reTemp='<div class="review">\
						...
						data-parentId="{f_id1}"data-username="{f_username1}">回复</p>\
					<div class="sonreview {f_id}"></div>\
					</div>',
		//评论的子评论的模板
		reTemp2='<div><p><span class="fontcolor">{f_username} </span>{f_title}：<span></span>{f_content}</span></span></p>\
		<textarea class="none1 none" rows="2"></textarea> \
		<p class="pfont inBlock">{f_post_time}</p><p class="fright hover2 pfont inBlock" data-parentId="{f_parent_id}"\
		data-username="{f_username1}">回复</p></div>';
		...
		if($(that).html()=="回复" && c!=""){
			...
			$.ajax({
		        ...
		        success:function(data){
		        	//成功的话插入评论
		        	console.log(data);
		        	var div='<div><p><span class="fontcolor">'+username+' </span>'+title+'：<span></span>'+content+'</span></span></p>\
		<textarea class="none1 none" rows="2"></textarea> \
		<p class="pfont inBlock">'+data[1]+'</p><p class="fright hover2 pfont inBlock" data-parentId="'+f_parent_id+'"\
		data-username="'+username+'">回复</p></div>';
		        ...
		        success:function(data){
			        ...
			        $.ajax({
			        	...
			        	var div='<div class="review">\
							...
							<p class="reContent">'+data[len].f_content+'</p>\
							<textarea class="none1 none" rows="2"></textarea> \
							<p class="pfont inBlock">发表于：<span>"'+data[len].f_post_time+'"</span></p><p class="fright hover1 pfont inBlock" \
							data-parentId="'+data[len].f_id+'"data-username="'+data[len].f_username+'">回复</p>\
